        ...
        commands = {
            ["play"] = {
                ...
                exec = function(message)
                    ...
                    if info.duration:toSeconds() > 600 and message.author ~= client.owner then
                        message:reply({
                            embed = {
                                title = "Music Player - `play`",
                    ...
                    local queue_message = message:reply({
                        embed = {
                    ...
                    mp_state[message.guild.id].thread_handle = coroutine.create(function()
                        while true do
                            ...
                            if uv.now() - song.queue_time > 12000 then
                                log:log(
                                    3,
                                    "Attempting to extract refreshed info for song: %s in %s",
                                    song.url,
                                    message.guild.name
                                )
                            ...
                            local function build_filter_chain()
                                ...
                                if mp_state[message.guild.id].tweaks.tempo > 2 then
                                    ...
                                    table.insert(
                                        filter_chain,
                                        "[tempo0] atempo=sqrt("
                                        .. mp_state[message.guild.id].tweaks.tempo
                                        .. ") [tempo];"
                                    )
                                else
                                    table.insert(
                                        filter_chain,
                                        "[0] atempo=" .. mp_state[message.guild.id].tweaks.tempo .. " [tempo];"
                                    )
                                end
                                ...
                                table.insert(
                                    filter_chain,
                                    "[norm] bass=g="
                                    .. mp_state[message.guild.id].tweaks.bassboost
                                    .. ":frequency=140 [bass];"
                                )
                                ...
                                if mp_state[message.guild.id].tweaks.bitcrush ~= 0 then
                                    table.insert(
                                        filter_chain,
                                        "[bass] acrusher=bits="
                                        .. mp_state[message.guild.id].tweaks.bitcrush
                                        .. ":mode=log [crush];"
                                    )
                                else
